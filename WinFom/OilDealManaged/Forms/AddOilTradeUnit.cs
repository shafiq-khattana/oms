...
{
    public partial class AddOilTradeUnit : Form
    {
        public int TradeUnitId = 0;
        public AddOilTradeUnit()
        {
            InitializeComponent();
        }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                OilTradeUnit tradeUnit = new OilTradeUnit
                {
                    Title = tbTitle.Text,                    
                ...
                {
                    var dbObj = db.OilTradeUnits.FirstOrDefault(a => a.Title.ToLower() == tradeUnit.Title.ToLower());
                    if(dbObj != null)
                    {
                        throw new Exception(string.Format("Trade unit with this name ({0}) already exists in database", dbObj.Title));
                    }

                    tradeUnit = db.OilTradeUnits.Add(tradeUnit);
                    db.SaveChanges();
