...
{
    public partial class AddOilDirtBrokerForm : Form
    {
        public int BrokerId = 0;
        public AddOilDirtBrokerForm()
        {
            InitializeComponent();
        }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                OilDirtBroker comp = new OilDirtBroker
                {
                ...
                {
                    var dbObj = db.OilDirtBrokers.ToList().FirstOrDefault(a => a.Equals(comp));
                    if (dbObj != null)
                    {
                    ...
                    using (var trans = db.Database.BeginTransaction())
                    {
                        ...
                        {
                            string na = "N/A";
                            string acctTitle = string.Format("{0} (oil dirt broker) account", comp.Name);
                            ...
                            string acctTitle2 = string.Format("{0} (oil dirt brokery expense) account", comp.Name);
                            ...
                            comp = db.OilDirtBrokers.Add(comp);
                            db.SaveChanges();
                            trans.Commit();
                            BrokerId = comp.Id;
