...
{
    public partial class CapitalAccountForm : Form
    {
        ...
        private string capAcctId = "";
        public bool IsDone = false;
        GeneralAccount capitalAccount = null;
        public CapitalAccountForm(string capitalAccountId)
        {
            InitializeComponent();
            capAcctId = capitalAccountId;
        }
        ...
        private void LoadTransactions()
        {
            try
            {
                ...
                {
                    capitalAccount = db.Accounts.Find(capAcctId) as GeneralAccount;

                    transactions = db.AccountTransactions.Where(a => a.GeneralAccountId == capAcctId)
                        .AsParallel().ToList()
        ...
        private void Form_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                lblHeading.Text = capitalAccount.Title;
            ...
            try
            {
                ...
                using (Context db = new Context())
                {
                    using (var trans = db.Database.BeginTransaction())
                    {
                        try
                        {
                            string message = string.Format("Cah amount ({0}) is introduced in business from ({1}). Added by ({2})", amount, capitalAccount.Title, SingleTon.LoginForm.appUser.Name);
            ...
            try
            {
                ...
                using (Context db = new Context())
                {
                    using (var trans = db.Database.BeginTransaction())
                    {
                        try
                        {
                            string message = string.Format("Amount ({0}) withdrawn from ({1}) by user ({1}).", amount, capitalAccount.Title, SingleTon.LoginForm.appUser.Name);
                            DayBook capitalEntryDayBook = new DayBook
                            {
                                Id = 0,
                                Date = DateTime.Now,
