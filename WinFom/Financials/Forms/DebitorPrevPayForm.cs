...
namespace WinFom.Financials.Forms
{
    ...
    public partial class DebitorPrevPayForm : Form
    {
        ...
        public DebitorPrevPayForm(string acctId)
        {
            InitializeComponent();
        ...
        private void Form_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                lblHeading.Text = string.Format("Add prev balance from ({0})", creditAccount.Title);
            ...
            {               
                ...
                using (Context db = new Context())
                {
                    using (var trans = db.Database.BeginTransaction())
                    {
                        ...
                        string finMsg = string.Format("Previous payable balance amount ({0}) is added in debitor account ({1}) from ({2}). Description ({3}) By ({4})",
                             amount.ToString("n1"),
                            creditAccount.Title, debitAccount.Title,
                        ...
                        Gujjar.InfoMsg("Previous balance entry is cleared and record is stored in database successfully");
                        IsDone = true;
                        Close();
