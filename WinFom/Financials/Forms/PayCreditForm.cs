...
namespace WinFom.Financials.Forms
{
    ...
    public partial class PayCreditForm : Form
    {
        ...
        public PayCreditForm(string acctId)
        {
            InitializeComponent();
            ...
            try
            {
                ...
                using (Context db = new Context())
                {
                    using (var trans = db.Database.BeginTransaction())
                    {
                        ...
                        debitAccount = db.Accounts.Find(creditAccountId) as GeneralAccount;
                        string tmsg = "";
                        if (bswIsBank.Value)
                        {
                        ...
                        string finMsg = string.Format("Liability paid: ({0}). Amount ({1}). Remarks ({2}), ({3}). By ({4})", debitAccount.Title, 
                        ...
                        trans.Commit();
                        Gujjar.InfoMsg("Liability is paid and record is stored in database successfully");
                        IsDone = true;
                        Close();
