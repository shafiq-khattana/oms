...
namespace WinFom.Financials.Forms
{
    ...
    public partial class DebitorPrevReceiveForm : Form
    {
        ...
        public DebitorPrevReceiveForm(string acctId)
        {
            InitializeComponent();
        ...
        private void Form_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                lblHeading.Text = debitAccount.Title;
            ...
            {               
                ...
                using (Context db = new Context())
                {
                    using (var trans = db.Database.BeginTransaction())
                    {
                        ...
                        string finMsg = string.Format("Opening balance entry, amount ({0}) is added in ({1}) from debitor ({2}), description ({3}). By ({4})",  
                            amount.ToString("n1"), creditAccount.Title, debitAccount.Title, tbDescription.Text, user.Name);
                        ...
                        Gujjar.InfoMsg("Previous receivable balance is added in opening balance and record is stored in database successfully");
                        IsDone = true;
                        Close();
